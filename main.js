...
async function initializeDatabase() {
  ...
  `);
    ...
    const hasValidationErrorMessageColumn = messageColumns.some((column) => column.name === 'validation_error_message');
...
function registerDbHandlers() {
    ...
    ipcMain.handle('db:createMessage', async (_event, message) => {
        const explicitMessageId = typeof message.id === 'number' && Number.isInteger(message.id) && message.id > 0
            ? message.id
            : undefined;
