...
export async function getAllLanguages(): Promise<LanguageFile[]> {
...
export async function listLanguages(): Promise<Omit<LanguageFile, 'strings'>[]> {
  return (await getAllLanguages()).map(({ strings, ...item }) => item);
}

export async function getLanguage(code: string): Promise<LanguageFile> {
  const language = (await getAllLanguages()).find((item) => item.code === code);
